...
class JWTAuthBackend:
    ...
    async def invalidate_token(self, token: str) -> None:
        ...
        """
        try:
            token_payload = self.jwt_handler.decode(token, verify=True)
        finally:
            await self.cache.delete(f"{token_payload['username']}:{token}")
    
    async def invalidate_token_of_user(self, username: str) -> None:
        """
        Invalidate a JWT token by removing it from the cache.
        ...
        """
        try:
            keys = await self.cache.keys(f"{username}:*")
            for key in keys:
                await self.cache.delete(key)
        except Exception as e:
            raise Exception(f"Failed to invalidate tokens of user: {e}")
    ...
    async def get_current_user(self, token: str) -> Optional[BaseModel]:
        ...
        try:
            cached_payload = await self.cache.get(f"{token_payload['username']}:{token}")
